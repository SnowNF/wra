...
main() {
    ...
    chroot rootfs /bin/su -l root -c /bin/bash -c "apt-get install -y libqt6widgets6 libqt6xml6" # contorl-panel
    ...
    chroot rootfs /bin/su -l root -c /bin/bash -c "usermod -a -G cdrom,floppy,sudo,audio,dip,video,plugdev,users,render,netdev,bluetooth wra"
    set +x
}
