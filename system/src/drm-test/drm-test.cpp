...
static int modeset_open(int *out, const char *node) {
    ...
    if (fd < 0) {
        ret = -errno;
        fprintf(stderr, "drm-test: cannot open '%s': %m\n", node);
        return ret;
    }

    if (drmGetCap(fd, DRM_CAP_DUMB_BUFFER, &has_dumb) < 0 || !has_dumb) {
        fprintf(stderr, "drm-test: drm device '%s' does not support dumb buffers\n",
                node);
        close(fd);
        return -EOPNOTSUPP;
    }
...
static struct modeset_dev *modeset_list = nullptr;
...
static int modeset_prepare(int fd) {
    ...
    if (!res) {
        fprintf(stderr, "drm-test: cannot retrieve DRM resources (%d): %m\n",
                errno);
        return -errno;
    }
    ...
    for (i = 0; i < res->count_connectors; ++i) {
        ...
        if (!conn) {
            fprintf(stderr, "drm-test: cannot retrieve DRM connector %u:%u (%d): %m\n",
        ...
        if (ret) {
            if (ret != -ENOENT) {
                errno = -ret;
                fprintf(stderr, "drm-test: cannot setup device for connector %u:%u (%d): %m\n",
                        i, res->connectors[i], errno);
            }
            free(dev);
...
static int modeset_setup_dev(int fd, drmModeRes *res, drmModeConnector *conn,
    ...
    if (conn->connection != DRM_MODE_CONNECTED) {
        fprintf(stderr, "drm-test: ignoring unused connector %u\n",
                conn->connector_id);
        return -ENOENT;
    }
    ...
    if (conn->count_modes == 0) {
        fprintf(stderr, "drm-test: no valid mode for connector %u\n",
                conn->connector_id);
        return -EFAULT;
    }
    ...
    fprintf(stderr, "drm-test: mode for connector %u is %ux%u\n",
            conn->connector_id, dev->width, dev->height);

    /* find a crtc for this connector */
    ret = modeset_find_crtc(fd, res, conn, dev);
    if (ret) {
        fprintf(stderr, "drm-test: no valid crtc for connector %u\n",
                conn->connector_id);
        return ret;
    }
    ...
    if (ret) {
        fprintf(stderr, "drm-test: cannot create framebuffer for connector %u\n",
                conn->connector_id);
        return ret;
    }
...
static int modeset_find_crtc(int fd, drmModeRes *res, drmModeConnector *conn,
    ...
    for (i = 0; i < conn->count_encoders; ++i) {
        ...
        if (!enc) {
            fprintf(stderr, "drm-test: cannot retrieve encoder %u:%u (%d): %m\n",
    ...
    fprintf(stderr, "drm-test: cannot find suitable CRTC for connector %u\n",
            conn->connector_id);
    return -ENOENT;
}
...
static int modeset_create_fb(int fd, struct modeset_dev *dev) {
    ...
    if (ret < 0) {
        fprintf(stderr, "drm-test: cannot create dumb buffer (%d): %m\n",
                errno);
        return -errno;
    }
    ...
    ret = drmModeAddFB(fd, dev->width, dev->height, 24, 32, dev->stride,
                       dev->handle, &dev->fb);
    if (ret) {
        fprintf(stderr, "drm-test: cannot create framebuffer (%d): %m\n",
                errno);
        ret = -errno;
        goto err_destroy;
    }
    ...
    if (ret) {
        fprintf(stderr, "drm-test: cannot map dumb buffer (%d): %m\n",
                errno);
        ret = -errno;
        goto err_fb;
    }
    ...
    dev->map = (uint8_t *) (mmap(nullptr, dev->size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, mreq.offset));
    if (dev->map == MAP_FAILED) {
        fprintf(stderr, "drm-test: cannot mmap dumb buffer (%d): %m\n",
                errno);
        ret = -errno;
        goto err_fb;
    }
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "drm-test: using card '%s'\n", card);
    ...
    for (iter = modeset_list; iter; iter = iter->next) {
        ...
        if (ret)
            fprintf(stderr, "drm-test: cannot set CRTC for connector %u (%d): %m\n",
                    iter->conn, errno);
    }
    ...
    if (ret) {
        errno = -ret;
        fprintf(stderr, "drm-test: modeset failed with error %d: %m\n", errno);
    } else {
        fprintf(stderr, "drm-test: exiting\n");
    }
    return ret;
}
