...
namespace android::init {
    ...
    class ColdBoot {
    public:
        ColdBoot(UeventListener &uevent_listener, std::vector<std::unique_ptr<UeventHandler>> &uevent_handlers)
        ...
        while (!subprocess_pids_.empty()) {
            ...
            if (WIFEXITED(status)) {
                if (WEXITSTATUS(status) == EXIT_SUCCESS) {
                    ...
                    wra_error("subprocess exited with status %d", WEXITSTATUS(status));
                }
            } else if (WIFSIGNALED(status)) {
                wra_error("subprocess killed by signal %d", WTERMSIG(status));
            }
        }
    }
    ...
    void ColdBoot::Run() {
        ...
        wra_info("Coldboot took %f seconds", duration.count() / 1000.0f);
    }

    int ueventd_main(int argc, char **argv) {
        umask(000);

        wra_info("ueventd started!");
        ...
        uevent_handlers.emplace_back(std::make_unique<ModaliasHandler>(
                std::move(ueventd_configuration.module_base_paths)));

        UeventListener uevent_listener(ueventd_configuration.uevent_socket_rcvbuf_size);

        if (uEvent_Config.cold_boot) {
        ...
        std::optional<std::chrono::milliseconds> relative_timeout;
        if (!uEvent_Config.loop)
            relative_timeout = std::chrono::milliseconds(500);
