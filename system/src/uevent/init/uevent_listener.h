...
namespace android::init {
    ...
    public:
        explicit UeventListener(size_t uevent_socket_rcvbuf_size);

        ~UeventListener();

        void RegenerateUevents(const ListenerCallback &callback) const;

        [[nodiscard]] ListenerAction RegenerateUeventsForPath(const std::string &path,
                                                              const ListenerCallback &callback) const;

        void Poll(const ListenerCallback &callback,
                  std::optional<std::chrono::milliseconds> relative_timeout = {}) const;
