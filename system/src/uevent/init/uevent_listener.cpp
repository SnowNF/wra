...
namespace android::init {
    static void ParseEvent(const char *msg, Uevent *uevent) {
        ...
        if (uEvent_Config.debug) {
            std::stringstream ss;
            ss << "event { '" << uevent->action << "', '" << uevent->path << "', '"
    ...
    UeventListener::UeventListener(size_t uevent_socket_rcvbuf_size) {
        device_fd = uevent_open_socket((int) uevent_socket_rcvbuf_size, true);
        if (device_fd == -1) {
            wra_fatal("Could not open uevent socket");
        }
    ...
    ReadUeventResult UeventListener::ReadUevent(Uevent *uevent) const {
        ...
        int n = (int) uevent_kernel_multicast_recv(device_fd, msg, UEVENT_MSG_LEN);
        if (n <= 0) {
            if (errno != EAGAIN && errno != EWOULDBLOCK && errno != EIO) {
                wra_error("Error reading from Uevent Fd %s", strerror(errno));
            }
            return ReadUeventResult::kFailed;
        }
        if (n >= UEVENT_MSG_LEN) {
            wra_error("Uevent overflowed buffer, discarding");
    ...
    ListenerAction UeventListener::RegenerateUeventsForDir(DIR *d, const ListenerCallback &callback) const {
        ...
        if (uEvent_Config.debug) {
            std::string path = "/proc/self/fd/" + std::to_string(dfd);
            std::string realPath;
            wra::Readlink(path, &realPath);
            wra_debug("Test %s", realPath.c_str());
        }
        ...
        while ((de = readdir(d)) != nullptr) {
            ...
            if (d2 == nullptr) {
                ...
                ListenerAction action = RegenerateUeventsForDir(d2, callback);
                closedir(d2);
                if (action == ListenerAction::kStop) {
    ...
    ListenerAction UeventListener::RegenerateUeventsForPath(const std::string &path,
        ...
        if (!dir) return ListenerAction::kContinue;

        auto r = RegenerateUeventsForDir(dir, callback);
        closedir(dir);
        return r;
    }
    ...
    void UeventListener::Poll(const ListenerCallback &callback,
        ...
        while (true) {
            ...
            if (relative_timeout) {
                ...
                timeout_ms = (int) remaining_timeout.count();
            }

            int nr = poll(&ufd, 1, timeout_ms);
            if (nr == 0) return;
            if (nr < 0) {
                wra_error("poll() of uevent socket failed, continuing");
                continue;
            }
