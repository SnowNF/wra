...
class Modprobe {
public:
    explicit Modprobe(const std::vector<std::string> &, bool use_blocklist = true);
    ...
    int GetModuleCount() const { return module_count_; }

    bool IsBlocklisted(const std::string &module_name);

private:
    static std::string MakeCanonical(const std::string &module_path);
    ...
    static std::string GetKernelCmdline();
    ...
    static void ParseCfg(const std::string &cfg, const std::function<bool(const std::vector<std::string> &)>& f);
